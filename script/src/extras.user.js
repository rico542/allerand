...
// @include			*.world-of-dungeons.fr/wod/spiel/hero/attributes.php*
// @version			1.10.1
// @author			Tomy
...
function AddTableRow(isHeader, index, data, center) {
    ...
    if (data.length > 0) {
        ...
        for (var j = 0; j < data.length; ++j) {
            var centertxt = "";
            if (center !== undefined) {
                if (typeof center === 'object' && center[j] || typeof center === "boolean" && center)
    ...
    for (var k in Data) {
        ...
        for (var ia = 0; ia < Data[k].length; ++ia) {
            if (!Data[k][ia].used_with)
                continue;
            txt += AddTableRow(false, j,
                               [k, Data[k][ia].value.Html() + "<sup>" + Contents.Used_With + "</sup>", Data[k][ia].value.Calc(HeroLevel).Html() + "<sup>" + Contents.Used_With + "</sup>", GetEquipmentHref(Data[k][ia].item, false)
                               ],
                               [false, true, true, false]
                              );
            j++;
...
function AddTableEx(heroID, Where, Data, heading, headers, negative)
{
    ...
    for (var k in Data) {
        for (var l in Data[k]) {
            ...
            if (cnt > 0) {
                txt += AddTableRow(false, j,
                                   [k, l, total.Html(negative) + "<sup>" + Contents.Dmg_With + "</sup>", total.Calc(HeroLevel).Html(negative) + "<sup>" + Contents.Dmg_With + "</sup>", (cnt > 1 ? disp : last_disp)
                                   ],
                                   [false, true, true, true, false]
                                  );
                j++;
            ...
            for (var ic = 0; ic < Data[k][l].length; ++ic) {
                if (!Data[k][l][ic].used_with)
                    continue;
                txt += AddTableRow(false, j,
                                   [k, l + "<sup>" + Contents.Used_With + "</sup>", Data[k][l][ic].value.Html(negative) + (Data[k][l][ic].in_addition ? "<sup>" + Contents.Dmg_With + "</sup>" : ""), Data[k][l][ic].value.Calc(HeroLevel).Html(negative) + (Data[k][l][ic].in_addition ? "<sup>" + Contents.Dmg_With + "</sup>" : ""), GetEquipmentHref(Data[k][l][ic].item, false)
                                   ],
                                   [false, true, true, true, false]
                                  );
                j++;
    ...
    if (!negative) {
        ...
        if (havea)
            txt += "<font size=\"-1\"><sup>(a)</sup> " + Contents.Item_Skill + "</font><br>";
        if (havez)
            txt += "<font size=\"-1\"><sup>(z)</sup> " + Contents.Damage_Added + "</font>";
    } else {
